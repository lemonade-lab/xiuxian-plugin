...
export class UserHome extends plugin {
    ...
    async refining(e) {
        ...
        pj = pj[code[1]];
        if (pj > 5 || thing_exist.atk < 10 && thing_exist.def < 10 && thing_exist.HP < 10) {
    ...
    async find_thing(e) {
        ...
        let didian = ["guildSecrets_list", "forbiddenarea_list", "Fairyrealm_list", "timeplace_list", "didian_list",
            "shenjie", "mojie", "xingge", "shop_list"];
        let found = [];
        let thing_exist = await foundthing(thing_name);
        if (!thing_exist) {
            e.reply(`你在瞎说啥呢?哪来的【${thing_name}】?`);
    ...
    async Player_use(e) {
        ...
        if (func == "服用") {
            ...
            if (thing_exist.type == "幸运") {
                ...
                data.setData("player", usr_qq, player);
                e.reply(`${thing_name}服用成功，将在之后的 ${quantity * 10}次冒险旅途中为你提高幸运值！`);
                await Add_najie_thing(usr_qq, thing_name, "丹药", -quantity);
                return;
            }
            if (thing_exist.type == '闭关') {
                ...
                e.reply(`${thing_name}提高了你的忍耐力,提高了下次闭关的效率,当前提高${dy.biguanxl * 100}%\n查看练气信息后生效`);
                await Add_najie_thing(usr_qq, thing_name, "丹药", -quantity);
            ...
            if (thing_exist.type == '凝仙') {
                if (dy.biguan > 0) { dy.biguan += thing_exist.机缘 * quantity }
                if (dy.lianti > 0) { dy.lianti += thing_exist.机缘 * quantity }
                if (dy.ped > 0) { dy.ped += thing_exist.机缘 * quantity }
                if (dy.beiyong2 > 0) { dy.beiyong2 += thing_exist.机缘 * quantity }
                e.reply(`丹韵入体,身体内蕴含的仙丹药效增加了${thing_exist.机缘 * quantity}次`);
                await Add_najie_thing(usr_qq, thing_name, "丹药", -quantity);
                await Write_danyao(usr_qq, dy);
                return;
            }
            if (thing_exist.type == '炼神') {
                ...
                e.reply(`服用了${thing_name},获得了炼神之力,下次闭关获得了炼神之力,当前炼神之力为${thing_exist.lianshen * 100}%`);
                await Write_danyao(usr_qq, dy);
                await Add_najie_thing(usr_qq, thing_name, "丹药", -quantity);
                return;
            }
            if (thing_exist.type == '神赐') {
                ...
                e.reply(`${player.名号}获得了神兽的恩赐,赐福的概率增加了,当前剩余次数${dy.beiyong2}`)
                await Write_danyao(usr_qq, dy);
                await Add_najie_thing(usr_qq, thing_name, "丹药", -quantity);
                return;
            }
            ...
            if (thing_exist.type == '器灵') {
                ...
                e.reply(`服用成功,您额外获得了${thing_exist.天赋 * quantity}天赋上限,您当前炼器天赋为${player.锻造天赋}`);
                await Write_player(usr_qq, player);
                await Add_najie_thing(usr_qq, thing_name, '丹药', -quantity);
                return;
            }
            if (thing_exist.type == '锻造上限') {
                ...
                e.reply(`服用成功,您下一次的炼器获得了额外的炼器格子[${thing_exist.额外数量}]`);
                await Add_najie_thing(usr_qq, thing_name, '丹药', -quantity);
                await Write_danyao(usr_qq, dy);
                return;
            }
            ...
            if (thing_exist.type == '入魔') {
                await Add_魔道值(usr_qq, quantity * thing_exist.modao);
                e.reply(`${quantity}道黑色魔气入体,增加了${quantity * thing_exist.modao}魔道值`);
                await Add_najie_thing(usr_qq, thing_name, "丹药", -quantity);
                return;
            }
            if (thing_exist.type == "补根") {
                player.灵根 = { "id": 70001, "name": "垃圾五灵根", "type": "伪灵根", "eff": 0.01, "法球倍率": 0.01 };
                data.setData("player", usr_qq, player);
            ...
            if (thing_exist.type == "补天") {
                player.灵根 = { "id": 70054, "name": "天五灵根", "type": "圣体", "eff": 0.20, "法球倍率": 0.12 };
                data.setData("player", usr_qq, player);
        ...
        if (func == "消耗") {
            ...
            if (qh) {
                ...
                else if (qh.class == "神人" && (player.魔道值 > 0 || (player.灵根.type != "转生" && player.level_id < 42))) {
