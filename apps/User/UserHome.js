...
export class UserHome extends plugin {
  ...
  async world(e) {
    ...
    for (let player_id of playerList) {
      ...
      else if ((player.lunhui > 0 || player.level_id > 41) && player.魔道值 < 1) num[0]++
  ...
  async refining(e) {
    ...
    //都通过了
    for (let i = 0; i < 3; i++) await Add_najie_thing(usr_qq, thing_name, '装备', -1, pj)
  ...
  async Take_lingshi(e) {
    ...
    if (func == '存') {
      ...
      e.reply([
        segment.at(usr_qq),
        `储存完毕,你目前还有${player_lingshi - lingshi}灵石,纳戒内有${najie.灵石 + lingshi}灵石`
      ])
      return false
    }
    if (func == '取') {
  ...
  async Player_use(e) {
    ...
    if (func == '服用') {
      ...
      if (thing_exist.type == '器灵') {
        ...
        e.reply(
          `服用成功,您额外获得了${thing_exist.天赋 * quantity}天赋上限,您当前炼器天赋为${
            player.锻造天赋
          }`
        )
        await Write_player(usr_qq, player)
    ...
    if (func == '消耗') {
      ...
      if (thing_exist.type == '洗髓') {
        ...
        e.reply([
          segment.at(usr_qq),
          `  服用成功,剩余 ${thing_name}数量: ${x - 1}，新的灵根为 "${player.灵根.type}"：${
            player.灵根.name
          }`,
          '\n可以在【#我的练气】中查看'
        ])
      ...
      if (qh) {
        if (qh.class == '魔头' && player.魔道值 < 1000) {
          ...
          qh.class == '神人' &&
          (player.魔道值 > 0 || (player.灵根.type != '转生' && player.level_id < 42))
        ) {
