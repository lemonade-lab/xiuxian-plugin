...
import {
    ...
    Numbers,
    Read_player,
    Read_najie, Write_najie,__PATH
} from "../Xiuxian/Xiuxian.js";

/**
 * 定时任务
 */
export class AssUnchartedPlaceTask extends plugin {
    constructor() {
        super({
            name: 'AssUnchartedPlaceTask',
            dsc: 'AssUnchartedPlaceTask',
            event: 'message',
            priority: 300,
            rule: [
            ]
        ...
        this.task = {
            ...
            name: 'AssUnchartedPlaceTask',
            fnc: () => this.AssUnchartedPlaceTask()
        }
    }

    async AssUnchartedPlaceTask() {
        let playerList = [];
        let files = fs
        ...
        for (let player_id of playerList) {
            let action = await redis.get("xiuxian:player:" + player_id + ":association:unchartedAction");
            ...
            if (action != null) {
                ...
                let assPlayer = assUtil.getAssOrPlayer(1,player_id);
                let ass = assUtil.getAssOrPlayer(2,assPlayer.assName);

                if (action.Ass_Uncharted_Action == "0") {
                    ...
                    if (now_time > end_time) {
                        ...
                        if (Data_battle.victory == player_id) {
                            ...
                            if (random1 <= x) {
                                ...
                                ass.spiritStoneAns -= searchsthing.price;
...
async function offaction(qq) {
    let action = await redis.get("xiuxian:player:" + qq + ":association:unchartedAction");
    action = JSON.parse(action);
    if (action != null) {
        await redis.del("xiuxian:player:" + qq + ":association:unchartedAction");
        let arr = action;
        ...
        arr.Ass_Uncharted_Action = 1;//秘境
        arr.Place_actionplus = 1;//沉迷状态
        arr.end_time = new Date().getTime();//结束的时间也修改为当前时间
        delete arr.group_id;//结算完去除group_id
        await redis.set("xiuxian:player:" + qq + ":association:unchartedAction", JSON.stringify(arr));
