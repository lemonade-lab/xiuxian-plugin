...
export class BOSS extends plugin {
    ...
    async ShowDamageList(e) {
        if (await BossIsAlive()) {
            ...
            await sleep(1000);
            if (CurrentQQ)
                e.reply(`你在妖王周本贡献排行榜中排名第${CurrentQQ}，造成伤害${PlayerRecord.TotalDamage[PlayerList[CurrentQQ - 1]]}，再接再厉！`);
        }
        else e.reply("妖王未开启！");
        return true;
    }
    ...
    async WorldBossBattle(e) {
        ...
        if (data.existData("player", usr_qq)) {
            ...
            if (random < 0.05 && msg.find(item => item == A_win)) {
                ...
                e.reply(`危及时刻,万先盟-韩立前来助阵,对[妖王]造成${Math.trunc(WorldBossStatus.Health * 0.15)}伤害,并治愈了你的伤势`);
            ...
            if (WorldBossStatus.Health <= 0) {
                ...
                await sleep(1000);
                let msg2 = "【全服公告】"+player.名号+"亲手结果了妖王的性命,为民除害,额外获得1000000灵石奖励！";
                ...
                Bot.logger.mark(`[妖王] 结算:${usr_qq}增加奖励1000000`);
                ...
                for (var i = 0; i < PlayerList.length; i++) {
                    ...
                    if (i < Show_MAX) {
                        ...
                        Bot.logger.mark(`[妖王周本] 结算:${PlayerRecordJSON.QQ[PlayerList[i]]}增加奖励${Reward}`);
                        continue;
                    }
                    else {
...
async function InitWorldBoss() {
    ...
    Bot.logger.mark(`[妖王] 化神玩家总数：${player_quantity}`);
    Bot.logger.mark(`[妖王] 生成基数:${X}`);
