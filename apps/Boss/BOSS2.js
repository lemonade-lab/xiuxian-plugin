...
import { Add_灵石, ForwardMsg, Add_HP } from '../Xiuxian/xiuxian.js'
    ...
    async ShowDamageList(e) {
        if (await BossIsAlive()) {
            ...
            for (var i = 0; i < PlayerList.length; i++) {
                if (i < 20) {
                    let Reward = Math.trunc((PlayerRecord.TotalDamage[PlayerList[i]] / TotalDamage) * WorldBossStatusStr.Reward);
            ...
            await sleep(1000);
            if (CurrentQQ)
                e.reply(`你在金角大王周本贡献排行榜中排名第${CurrentQQ}，造成伤害${PlayerRecord.TotalDamage[PlayerList[CurrentQQ - 1]]}，再接再厉！`);
        }
        else e.reply("金角大王未开启！");
        return true;
    }
    ...
    async WorldBossBattle(e) {
        ...
        if (data.existData("player", usr_qq)) {
            ...
            if (WorldBOSSBattleLock != 0) {
                e.reply("好像有人正在和银角大王激战，现在去怕是有未知的凶险，还是等等吧！");
                return true;
            }
            ...
            if (msg.find(item => item == A_win)) {
                TotalDamage = Math.trunc(WorldBossStatus.Healthmax * 0.06 + Harm(player.攻击 * 0.85, Boss.防御) * 10);
                WorldBossStatus.Health -= TotalDamage;
                e.reply(`${player.名号}击败了[${Boss.名号}],重创[金角大王],造成伤害${TotalDamage}`);
            }
            else if (msg.find(item => item == B_win)) {
                TotalDamage = Math.trunc(WorldBossStatus.Healthmax * 0.04 + Harm(player.攻击 * 0.85, Boss.防御) * 6);
                WorldBossStatus.Health -= TotalDamage;
                e.reply(`${player.名号}被[${Boss.名号}]击败了,只对[金角大王]造成了${TotalDamage}伤害`);
            }
            await Add_HP(usr_qq, Data_battle.A_xue);
            await sleep(1000);
            let random = Math.random();
            if (random < 0.05 && msg.find(item => item == A_win)) {
                e.reply("这场战斗重创了[金角大王]，金角大王使用了古典秘籍,血量回复了10%");
                ...
                e.reply(`危及时刻,万先盟-韩立前来助阵,对[金角大王]造成${Math.trunc(WorldBossStatus.Health * 0.15)}伤害,并治愈了你的伤势`);
            ...
            if (WorldBossStatus.Health <= 0) {
                ...
                await sleep(1000);
                let msg2 = "【全服公告】" + player.名号 + "亲手结果了金角大王的性命,为民除害,额外获得500000灵石奖励！";
                ...
                for (var i = 0; i < PlayerList.length; i++) {
                    ...
                    if (i < Show_MAX) {
                        ...
                        Bot.logger.mark(`[金角大王周本] 结算:${PlayerRecordJSON.QQ[PlayerList[i]]}增加奖励${Reward}`);
                        continue;
                    }
                    else {
                        CurrentPlayer.灵石 += 200000;
                        Bot.logger.mark(`[金角大王周本] 结算:${PlayerRecordJSON.QQ[PlayerList[i]]}增加奖励200000`);
                        data.setData("player", PlayerRecordJSON.QQ[PlayerList[i]], CurrentPlayer);
                    }
...
async function InitWorldBoss() {
    ...
    Bot.logger.mark(`[金角大王] 化神玩家总数：${player_quantity}`);
    Bot.logger.mark(`[金角大王] 生成基数:${X}`);
