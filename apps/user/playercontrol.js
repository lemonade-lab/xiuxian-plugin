...
export class playercontrol extends plugin {
  ...
  async chuGuan(e) {
    ...
    let y = xiuxiandata.biguan.time //固定时间
    let x = xiuxiandata.biguan.cycle //循环次数
  ...
  async endWork(e) {
    ...
    let y = xiuxaindata.work.time //固定时间
    let x = xiuxaindata.work.cycle //循环次数
  ...
  async biguan_jiesuan(user_id, time, is_random, group_id) {
    ...
    let size = config.getconfig('xiuxian', 'xiuxian').biguan.size
    //增加的修为
  ...
  async dagong_jiesuan(user_id, time, is_random, group_id) {
    ...
    let size = config.getconfig('xiuxian', 'xiuxian').work.size
