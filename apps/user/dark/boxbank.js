...
export class BoxBank extends plugin {
    ...
    moneyWorkshop = async (e) => {
        ...
        const StorageList = await GameApi.UserData.listActionInitial({ NAME: 'storage', CHOICE: 'user_bank', INITIAL: {} })
        const WhiteBarList = await GameApi.UserData.listActionInitial({ NAME: 'whiteBar', CHOICE: 'user_bank', INITIAL: {} })
        const msg = []
    ...
    substitution = async (e) => {
        ...
        await GameApi.GameUser.userBag({ UID, name: '下品灵石', ACCOUNT: -Number(new_account) })
        await GameApi.GameUser.userBag({ UID, name: new_name, ACCOUNT: Number(new_money) })
    ...
    storage = async (e) => {
        ...
        const account = await GameApi.GamePublic.leastOne({ value: e.msg.replace('#金银存储', '') })
        const money = await GameApi.GameUser.userBagSearch({ UID, name: '下品灵石' })
        ...
        if (StorageList.hasOwnProperty(UID)) {
            ...
            money += account
            await GameApi.GameUser.userBag({ UID, name: '下品灵石', ACCOUNT: - Number(account) })
            const isreply = await e.reply(`${UID}成功再存储${account}下品灵石,当前存储总金额${money}`)
            await BotApi.User.surveySet({ e, isreply })
            StorageList[UID].account = money
            await GameApi.UserData.listActionInitial({ NAME: 'storage', CHOICE: 'user_bank', INITIAL: {}, DATA: StorageList })
            return
        }
        StorageList[UID] = {
            account,
    ...
    withdraw = async (e) => {
        ...
        const account = await GameApi.GamePublic.leastOne({ value: await GameApi.GamePublic.leastOne({ value: e.msg.replace('#金银取出', '') }) })
        if (account < 5000) {
            e.reply(`[金银坊]金老三\n取出少于5000`)
            return
        }
    ...
    whiteBar = async (e) => {
        ...
        const money = await GameApi.GamePublic.leastOne({ value: e.msg.replace('#金银白条', '') })
        if (money < 10000) {
            e.reply(`白条不足10000`)
            return
        }
        ...
        await GameApi.UserData.listActionInitial({ NAME: 'whiteBar', CHOICE: 'user_bank', INITIAL: {}, DATA: WhiteBarList })
        await GameApi.GameUser.userBag({ UID, name: '下品灵石', ACCOUNT: Number(money) })
        e.reply(`成功白条${money}`)
        return
    }
    ...
    deleteWhiteBar = async (e) => {
        ...
        const money = await GameApi.GamePublic.leastOne({ value: e.msg.replace('#金银消条', '') })
        const bag_money = await GameApi.GameUser.userBagSearch({ UID, name: '下品灵石' })
