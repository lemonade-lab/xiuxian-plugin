...
export class BoxExchange extends plugin {
    ...
    supermarket = async (e) => {
        ...
        const exchange = await GameApi.UserData.listActionInitial({ NAME: 'exchange', CHOICE: 'generate_exchange', INITIAL: [] })
        const msg = [
            '___[虚空镜]___'
        ]
    ...
    onsell = async (e) => {
        ...
        const bagThing = await GameApi.GameUser.userBagSearch({ UID, name: thing_name })
        if (!bagThing) {
        ...
        await GameApi.UserData.listActionInitial({ NAME: 'exchange', CHOICE: 'generate_exchange', DATA: exchange, INITIAL: [] })
        await GameApi.GameUser.userBag({ UID, name: bagThing.name, ACCOUNT: -Number(account) })
    ...
    Offsell = async (e) => {
        ...
        let x = 888888888
        let exchange = await GameApi.UserData.listActionInitial({ NAME: 'exchange', CHOICE: 'generate_exchange', INITIAL: [] })
        ...
        await GameApi.GameUser.userBag({ UID, name: exchange[x].thing.name, ACCOUNT: Number(exchange[x].account) })
    ...
    purchase = async (e) => {
        ...
        await GameApi.GameUser.userBag({ UID, name: exchange[x].thing.name, ACCOUNT: Number(exchange[x].account) })
        await GameApi.GameUser.userBag({ UID, name: '下品灵石', ACCOUNT: -Number(exchange[x].money) })
        await GameApi.GameUser.userBag({ UID: exchange[x].UID, name: '下品灵石', ACCOUNT: Number(exchange[x].money) })
        exchange = exchange.filter(item => item.ID != ID)
        await GameApi.UserData.listActionInitial({ NAME: 'exchange', CHOICE: 'generate_exchange', DATA: exchange, INITIAL: [] })
        e.reply(`成功选购${ID}`)
        return
