...
export class BoxEquipment extends plugin {
  ...
  synthesis = async (e) => {
    ...
    ans.elixirPercent =
      materialA.elixirPercent +
      materialB.elixirPercent +
      materialC.elixirPercent;
    ans.equipPercent =
      materialA.equipPercent + materialB.equipPercent + materialC.equipPercent;
    //成功率
    ...
    if (ans.elixirPercent >= 2 * ans.equipPercent) {
      ...
      //烂
      e.reply(
        `材料的属性产生了冲突,不知道变成即无法成丹,也无法成器,你获得了无用的残渣`
      );
