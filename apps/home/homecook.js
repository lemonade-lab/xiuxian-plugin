...
export class homecook extends plugin {
  ...
  async choose_food(e) {
    ...
    useraction[UID] = setTimeout(async () => {
      ...
      if (shipu.proficiency != undefined) {
        shipu.proficiency -= 1;
        e.reply(
          `恭喜你，成功炒出【${recipes1.name
          }】，消耗${nameIwant}一点耐久度，试用食谱熟练度当前为${100 - shipu.proficiency
          }`
        );
      } else {
        shipu.durable -= 1;
  ...
  async choose_food1(e) {
    ...
    let cook = await HomeApi.Listdata.listActionInitial({
      CHOICE: "user_cook",
      NAME: "cook",
      INITIAL: data,
    });
    ...
    useraction[UID] = setTimeout(async () => {
      ...
      let food_thing = await HomeApi.Listdata.listActionInitial({
        CHOICE: "user_food",
        NAME: "food",
        INITIAL: data,
      });
  ...
  fabucaipu = async (e) => {
    ...
    const action = await GameApi.UserData.listAction({ CHOICE: 'user_action', NAME: UID })
    ...
    Warehouse = await HomeApi.GameUser.Add_DATA_thing({ DATA: Warehouse, DATA1: caipu, quantity: -1 })
    Warehouse = await HomeApi.GameUser.Add_DATA_thing({ DATA: Warehouse, DATA1: target, quantity: 1 })
    ...
    e.reply(`恭喜${UID}成功在万民堂发布一份食谱，玩家可前往万民堂购买，发布者可获得50%出售收益的版权费`)
    return
  }
  ...
  wanminbug = async (e) => {
    ...
    let Warehouse = await HomeApi.Listdata.listActionArr({ CHOICE: 'user_Warehouse', NAME: UID })
    Warehouse = await HomeApi.GameUser.Add_DATA_thing({ DATA: Warehouse, DATA1: ifexist, quantity: 1 })
    await HomeApi.Listdata.listActionArr({ CHOICE: 'user_Warehouse', NAME: UID, DATA: Warehouse })
    await HomeApi.GameUser.Add_doge({ UID, money: -commodities_doge })
    if (ifexist.qq == UID) {
      e.reply(`感谢您的购买，这次税率为【${rand}】,最终花了[${commodities_doge}]灵晶从万民堂购买了[${thing_name}]`)
      return
    } else {
      await HomeApi.GameUser.Add_doge({ UID: ifexist.qq, money })
      e.reply(`感谢您的购买，这次税率为【${rand}】,最终花了[${commodities_doge}]灵晶从万民堂购买了[${thing_name}]，食谱提供者：${ifexist.qq} 获得了${money}版权费`)
      return
    }
  }
  ...
  stir_fry = async (e) => {
    ...
    useraction[UID] = setTimeout(async () => {
      ...
      if (shipu1.proficiency != undefined) {
        ...
        e.reply(
          `恭喜你，成功炒出【${quantity}】份【${recipes1.name}】，消耗${thing1[0]}【${quantity}】点耐久度，试用食谱熟练度当前为${100 - shipu1.proficiency}`
        );
      } else {
