...
export class homeminefield extends plugin {
    ...
    if (minefield_name1 == undefined) {
      ...
      useraction[UID] = setTimeout(async () => {
        ...
        if (QQ != B) {
          ...
          e.reply(
            `你被矿主胖揍一顿，并且被他搜刮了${lingshi1}灵石作为赔偿，获得家园经验70`
          );
        }
      }, 1000 * time1);
  ...
  coal = async (e) => {
    ...
    useraction[UID] = setTimeout(async () => {
      ...
      let Warehouse_thing =
        await HomeApi.GameUser.homeexist_Warehouse_thing_name({
          UID,
          name: thing_name,
        });
      if (Warehouse_thing == 1 || Warehouse_thing.acount < quantity) {
  ...
  resolve = async (e) => {
    ...
    let sp = ["地磁道芯", "九玄天晶", "天工秘物", "神兵碎片", "天造道芯", "八卦天晶", "离火秘物", "乾坤碎片"]
