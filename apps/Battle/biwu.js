  ...
  async battle(e, num) {
    ...
    while (A_player.当前血量 > 0 && B_player.当前血量 > 0) {
      ...
      if (action_A.use != 4) {
        ...
        B_player.当前血量 -= A_伤害;
        msgg.push(
          `第${cnt}回合,${A_player.名号}触发${
            action_A[`技能${action_A.use}`]
          }，${ifbaoji(A_baoji)}造成伤害${A_伤害}，${B_player.名号}剩余血量${
            B_player.当前血量
          }\n`
        );
      } else {
      ...
      if (action_B.use != 4) {
        ...
        A_player.当前血量 -= B_伤害;
        msgg.push(
          `第${cnt}回合,${B_player.名号}触发${
            action_B[`技能${action_B.use}`]
          }，${ifbaoji(B_baoji)}造成伤害${B_伤害}，${A_player.名号}剩余血量${
            A_player.当前血量
          }`
        );
      } else {
