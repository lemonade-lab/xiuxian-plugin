...
import fs from "fs"
import { get_ranking_power_img, get_ranking_money_img } from '../../model/information.js'
import { Read_player, Read_najie, existplayer, Get_xiuwei, sortBy, sleep, ForwardMsg, isNotNull, __PATH } from '../../model/xiuxian.js'
...
export class toplist extends plugin {
    ...
    async TOP_Immortal(e) {
        ...
        for (let player_id of playerList) {
            ...
            //计算并保存到数组
            let power = (player.攻击 + player.防御 * 0.8 + player.血量上限 * 0.6) * player.暴击率;
    ...
    async TOP_genius(e) {
        ...
        for (let player_id of playerList) {
            ...
            let player = await Read_player(player_id);
            //计算并保存到数组
            let power = (player.攻击 + player.防御 * 0.8 + player.血量上限 * 0.5) * player.暴击率;
            if (player.level_id >= 42) {
                //跳过仙人的记录
                continue;
            }
