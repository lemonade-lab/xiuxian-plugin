...
export class games extends plugin {
  ...
  async Moneynumber(e) {
    ...
    //设置
    let time = config.getconfig('xiuxian', 'xiuxian').CD.gambling //
    //获取当前时间
  ...
  async Moneycheckguess(e) {
    ...
    if ((es == '大' && touzi > 3) || (es == '小' && touzi < 4)) {
      //赢了
      //获奖倍率
      let x = configdata.percentage.Moneynumber
      let y = 1
      let z = configdata.size.Money * 10000
      //增加金银坊赌博记录
      //押注大于一百万
      if (yazhu[usr_qq] >= z) {
        //扣一半的押注
  ...
  async Couple(e) {
    ...
    let Time = config.getconfig('xiuxian', 'xiuxian').CD.couple //6个小时
