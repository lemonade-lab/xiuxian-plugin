...
export class games extends plugin {
    ...
    async Xiuianplay(e) {
        ...
        if (rand < 0.5) {
            ...
            e.reply("花费了" + money + "灵石,你好好放肆了一番,奇怪的修为增加了" + randexp + "!在鱼水之欢中你顿悟了,修为增加了" + addlevel + "!");
            await Add_修为(usr_qq, addlevel);
            await Add_灵石(usr_qq, -money);
            let gameswitch = config.getconfig("xiuxian", "xiuxian").switch.Xiuianplay_key;
            if (gameswitch == true) {
                setu(e);
            }
            return;
    ...
    async Couple(e) {
        ...
        if (A != B) {
            ...
            if (option > 0 && option <= 0.5) {
                ...
                await Add_修为(A, -parseInt(y));
                await Add_修为(B, -parseInt(y));
                e.reply('你们双修时听到一声鸡叫，仿佛鸡说了句“你干嘛，哎哟~”，吓得你们心神不稳，都失去了' + parseInt(y) + '修为,是不是肾虚了，快去买药补补身子吧');
