...
export class sudokugame extends plugin {
  ...
  async CreateSudoku(e) {
    ...
    /** 回复 */
    await e.reply("请发送【简单】【中等】【困难】选择生成棋局的难度", false, {
      at: true,
    });
  }
  ...
  makeDigits() {
    ...
    for (i = 0; i < 8; i++) {
      ...
      for (j = 0; j < 9; j++) {
        ...
        error = false;
        while (
          colLists[j].includes(nine[0]) ||
          areaLists[areaIndex].includes(nine[0])
        ) {
...
function isValidSudoku(board) {
  ...
  for (let i = 0; i < 9; i++) {
    for (let j = 0; j < 9; j++) {
      for (let z = j + 1; z < 9; z++) {
        if (board[i][j] != 0 && board[i][j] === board[i][z]) {
          //判断行是否满足规则
          flag = 1;
          return false;
        }
        if (board[j][i] != 0 && board[j][i] === board[z][i]) {
          //判断列是否满足规则
          flag = 1;
          return false;
        }
...
function solveSudoku(board_solve) {
  ...
  let hasConflict = (r, c, val) => {
    ...
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
        if (board_solve[subRowStart + i][subColStart + j] == val) {
          //判断它所在小框是否重复
          return true;
        }
      }
  ...
  let fill = (i, j) => {
    ...
    if (board_solve[i][j] != 0) {
      //如果不为空,执行下一个
      return fill(i, j + 1);
    }
    for (let num = 1; num <= 9; num++) {
      //开始填入数字
      if (hasConflict(i, j, num)) {
        //冲突
        continue;
      }
...
async function reward(e) {
  ...
  for (let i in allqq) {
    ...
    e.reply([
      segment.at(allqq[i]),
      ` 你参与完成了${logfile.difficulty}级棋局,获得${lingshi}灵石`,
    ]);
  }
  return;
}
