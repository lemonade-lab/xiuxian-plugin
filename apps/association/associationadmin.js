import { plugin, dsc } from '../../api/api.js'
...
export class associationadmin extends plugin {
  ...
  async weihu(e) {
    ...
    e.reply(
      `维护成功,宗门还有${ass.灵石池}灵石,护宗大阵增加了${lingshi * 20}血量`
    )
  }
  ...
  async Set_appointment(e) {
    ...
    //检索输入的第一个职位
    let reg = new RegExp(/长老|外门弟子|内门弟子/)
    ...
    e.reply([
      ...
      `${ass.宗门名称} 宗主已经成功将${member.名号}任命为${appointment}!`
    ])
    return false
  }
  ...
  async Maintenance(e) {
    ...
    if (ass.灵石池 < ass.宗门等级 * 10000) {
      e.reply(
        `目前宗门维护需要${ass.宗门等级 * 10000}灵石,本宗门灵石池储量不足`
      )
      return false
    }
