...
class gameBattle {
  ...
  monsterbattle = async (parameter) => {
    ...
    if (battleA.speed >= battleB.speed - 5) {
      battle_hurt.hurtA =
        battleA.attack - battleB.defense >= 0
          ? battleA.attack - battleB.defense + 1
          : 0;
      ...
      if (T) {
        battle_hurt.hurtA += Math.floor(
          (battle_hurt.hurtA * battleA.burstmax) / 100
        );
      }
    ...
    while (true) {
      ...
      battle_hurt.hurtB =
        battleB.attack - battleA.defense >= 0
          ? battleB.attack - battleA.defense + 1
          : 0;
      const F = await this.probability(battleB.burst);
      if (F) {
        battle_hurt.hurtB += Math.floor(
          (battle_hurt.hurtB * battleB.burstmax) / 100
        );
      }
  ...
  battle = async (parameter) => {
    ...
    if (battleA.speed >= battleB.speed - 5) {
      ...
      if (battle_hurt.hurtA <= 1) {
        battle_msg.msg.push(
          "你个老六想偷袭,却连对方的防御都破不了,被对方一巴掌给拍死了!"
        );
        battleA.nowblood = 0;
        battle_msg.QQ = B;
    ...
    while (true) {
      ...
      if (battle_hurt.hurtB > 1) {
        if (battleA.nowblood < 0) {
          battle_msg.msg.push(
            `第${battle.Z}回合:对方造成${battle_hurt.hurtB}伤害`
          );
          battleA.nowblood = 0;
          battle_msg.QQ = B;
          await BotApi.User.forwardMsg({ e, data: battle_msg.msg });
      ...
      if (battleB.nowblood < 0) {
        battle_msg.msg.push(
          `第${battle.Z}回合:你造成${battle_hurt.hurtA}伤害,并击败了对方!`
        );
