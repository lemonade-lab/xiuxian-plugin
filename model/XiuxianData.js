...
class XiuxianData {
  constructor() {
    ...
    //加载强化列表
    this.qianghua = JSON.parse(fs.readFileSync(`${this.lib_path}/强化列表.json`))
    //锻造材料列表
    this.duanzhaocailiao = JSON.parse(fs.readFileSync(`${this.lib_path}/锻造材料.json`))
    //锻造武器列表
    this.duanzhaowuqi = JSON.parse(fs.readFileSync(`${this.lib_path}/锻造武器.json`))
    //锻造护具列表
    this.duanzhaohuju = JSON.parse(fs.readFileSync(`${this.lib_path}/锻造护具.json`))
    //锻造宝物列表
    this.duanzhaobaowu = JSON.parse(fs.readFileSync(`${this.lib_path}/锻造宝物.json`))
    //隐藏灵根列表
    this.yincang = JSON.parse(fs.readFileSync(`${this.lib_path}/隐藏灵根.json`))
    //锻造杂类列表
