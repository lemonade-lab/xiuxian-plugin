...
export async function player_efficiency(usr_qq) {
  ...
  if (!isNotNull(player.学习的功法)) {
    ...
    for (let i = 0; i < player.学习的功法.length; i++) {
      //存在功法，遍历功法加成
      ...
      try {
        ...
        if (ifexist2 == undefined) {
          ifexist2 = data.timegongfa_list.find(
            (item) => item.name == gongfa_name
          )
        }
      } catch {
  ...
  player.修炼效率提升 =
    linggen_efficiency + Assoc_efficiency + gongfa_efficiency //修炼效率综合
...
export async function instead_equipment(usr_qq, thing_name) {
  ...
  try {
    //根据名字找类型
    thing_type = data.equipment_list.find(
    ...
    if (thing_type == '武器') {
      ...
      //根据名字找武器
      equipment.武器 = data.equipment_list.find(
        (item) => item.name == thing_name
      )
      //武器写入装备
    ...
    if (thing_type == '护具') {
      ...
      equipment.护具 = data.equipment_list.find(
        (item) => item.name == thing_name
      )
      await Write_equipment(usr_qq, equipment)
      return
    ...
    if (thing_type == '法宝') {
      ...
      equipment.法宝 = data.equipment_list.find(
        (item) => item.name == thing_name
      )
      await Write_equipment(usr_qq, equipment)
      return
    ...
    if (thing_type == '武器') {
      ...
      await Add_najie_thing(usr_qq, equipment.武器.name, '装备', 1)
      //根据名字找武器，
      equipment.武器 = data.timeequipmen_list.find(
        (item) => item.name == thing_name
      )
      //把武器装备起来
      await Write_equipment(usr_qq, equipment)
    ...
    if (thing_type == '护具') {
      await Add_najie_thing(usr_qq, equipment.护具.name, '装备', 1)
      equipment.护具 = data.timeequipmen_list.find(
        (item) => item.name == thing_name
      )
      await Write_equipment(usr_qq, equipment)
      return
    ...
    if (thing_type == '法宝') {
      await Add_najie_thing(usr_qq, equipment.法宝.name, '装备', 1)
      equipment.法宝 = data.timeequipmen_list.find(
        (item) => item.name == thing_name
      )
      await Write_equipment(usr_qq, equipment)
      return
