  ...
  async battle(e, num) {
    ...
    while (A_player.now_bool > 0 && B_player.now_bool > 0) {
      ...
      action_A = await JSON.parse(
        await redis.get('xiuxian@1.4.0:' + A_QQ[num].QQ + ':bisai')
      )
      //清空cd
      ...
      const jineng = data.jineng()
      if (buff_B.玄冰封印) {
        const bj = jineng.find((item) => item.name == `玄冰封印`).pr
        A_player.暴击率 = bj
        buff_B.玄冰封印--
        msg.push(
      ...
      if (buff_A.心烦意乱) {
        const def = jineng.find((item) => item.name == `心烦意乱`).pr
        B_player.防御 *= 1 - def
        buff_A.心烦意乱--
        msg.push(
      ...
      if (buff_A.失魂落魄) {
        const def = jineng.find((item) => item.name == `失魂落魄`).pr
        B_player.防御 *= 1 - def
        buff_A.失魂落魄--
        msg.push(
      ...
      action_B = await JSON.parse(
        await redis.get('xiuxian@1.4.0:' + B_QQ[num].QQ + ':bisai')
      )
      //清空cd
      ...
      const jinengs = data.jineng()
      if (buff_A.阴风蚀骨) {
        const atk = jinengs.find((item) => item.name == `阴风蚀骨`).pr
        B_player.攻击 *= 1 - atk
        buff_A.阴风蚀骨--
        msg.push(
      ...
      if (buff_A.万年俱灰) {
        const atk = jinengs.find((item) => item.name == `万年俱灰`).pr
        B_player.攻击 *= 1 - atk
        buff_A.万年俱灰--
        msg.push(
      ...
      if (buff_A.玄冰封印) {
        const bj = jinengs.find((item) => item.name == `玄冰封印`).pr
        B_player.暴击率 = bj
        buff_A.玄冰封印--
        msg.push(
      ...
      if (buff_B.心烦意乱) {
        const def = jinengs.find((item) => item.name == `心烦意乱`).pr
        A_player.防御 *= 1 - def
        buff_B.心烦意乱--
        msg.push(
      ...
      if (buff_B.失魂落魄) {
        const def = jinengs.find((item) => item.name == `失魂落魄`).pr
        A_player.防御 *= 1 - def
        buff_B.失魂落魄--
        msg.push(
      ...
      if (buff_B.祝水咒) {
        const hp = jinengs.find((item) => item.name == `祝水咒`).pr
      ...
      if (buff_B.诛仙三剑) {
        const harm = jinengs.find((item) => item.name == `诛仙三剑`).pr
        B_伤害 *= 1 + harm
        buff_B.诛仙三剑--
        msg.push(
