...
export class BOSS2 extends plugin {
  ...
  async DeleteWorldBoss(e) {
    if (e.isMaster) {
      if (await BossIsAlive()) {
        ...
        await redis.del('xiuxian@1.4.0Record2')
        e.reply('金角大王挑战关闭！')
  ...
  async ShowDamageList(e) {
    if (await BossIsAlive()) {
      ...
      let PlayerRecord = JSON.parse(await redis.get('xiuxian@1.4.0Record2'))
  ...
  async WorldBossBattle(e) {
    ...
    if (data.existData('player', user_id)) {
      ...
      let PlayerRecord = await redis.get('xiuxian@1.4.0Record2')
      ...
      let Boss = {
        name: '银角大王',
        攻击: player.攻击 * (0.8 + 0.4 * Math.random()),
        防御: player.防御 * (0.8 + 0.4 * Math.random()),
        now_bool: player.血量上限 * (0.8 + 0.4 * Math.random()),
        暴击率: player.暴击率,
      ...
      redis.set('xiuxian@1.4.0Record2', JSON.stringify(PlayerRecordJSON))
      ...
      if (WorldBossStatus.Health <= 0) {
        ...
        let msg2 =
          '【全服公告】' +
          player.name +
          '亲手结果了金角大王the性命,为民除害,额外获得500000money奖励！'
        ...
        for (let i = 0; i < PlayerList.length; i++) {
          ...
          if (i == PlayerList.length - 1)
            Rewardmsg.push('其余参与the修仙者均获得200000money奖励！')
        }
...
async function InitWorldBoss() {
  ...
  console.error(`[金角大王] 化神玩家总数：${player_quantity}`)
  console.error(`[金角大王] 生成基数:${X}`)
  ...
  await redis.set('xiuxian@1.4.0Record2', JSON.stringify(PlayerRecord))
  let msg = '【全服公告】金角大王已经苏醒,击杀者额外获得50wmoney'
  const redisGlKey = 'xiuxian:AuctionofficialTask_GroupList'
  const groupList = await redis.sMembers(redisGlKey)
  for (const group_id of groupList) {
...
async function BossIsAlive() {
  return (
    (await redis.get('Xiuxian:WorldBossStatus2')) &&
    (await redis.get('xiuxian@1.4.0Record2'))
  )
}
...
async function GetAverageDamage() {
  ...
  for (let i = 0; i < File.length; i++) {
    ...
    if (player.level_id > 21 && player.level_id < 42 && player.lunhui == 0) {
      ...
      console.error(`[金角大王] ${this_qq}玩家攻击:${temp[TotalPlayer]}`)
      TotalPlayer++
    }
  }
  //排序
