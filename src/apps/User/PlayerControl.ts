...
export class PlayerControl extends plugin {
  ...
  async biguan_jiesuan(user_id, time, is_random, group_id = undefined) {
    ...
    let qixue = 0
    if (
      (await exist_najie_thing(user_id, '魔界秘宝', '道具')) &&
      player.魔道值 > 999
    ) {
      other_x = Math.trunc(xiuwei * 0.15 * time)
      await Add_najie_thing(user_id, '魔界秘宝', '道具', -1)
      msg.push('\n消耗了道具[魔界秘宝],额外增加' + other_x + 'now_exp')
      await Add_now_exp(user_id, other_x)
    }
    if (
      (await exist_najie_thing(user_id, '神界秘宝', '道具')) &&
      player.魔道值 < 1 &&
      (player.talent.type == '转生' || player.level_id > 41)
    ) {
      qixue = Math.trunc(xiuwei * 0.1 * time)
      await Add_najie_thing(user_id, '神界秘宝', '道具', -1)
      msg.push('\n消耗了道具[神界秘宝],额外增加' + qixue + '血气')
      await Add_血气(user_id, qixue)
    }
    //设置now_exp，设置血量

    await setFileValue(user_id, blood * time, 'now_bool')
    ...
    if (transformation == '血气') {
      ...
      await setFileValue(user_id, xiuwei * time + other_xiuwei, transformation)
      if (is_random) {
        msg.push(
          '\n增加气血:' + xiuwei * time,
  ...
  async dagong_jiesuan(user_id, time, is_random, group_id = undefined) {
    ...
    //设置money
    await setFileValue(user_id, get_lingshi, 'money')
