...
export class Level extends plugin {
  ...
  async LevelMax_up(e, luck) {
    ...
    if (rand > prob) {
      ...
      if (bad_time > 0.9) {
        ...
        e.reply(
          `突破瓶颈时想起怡红院里the放肆,想起了金银坊里the狂热,险些走火入魔，丧失了` +
            need_exp * 0.2 +
            '血气',
          false,
          { at: true }
  ...
  async Level_up(e, luck = false) {
    ...
    if (
      now_level_id >= 51 &&
      player.talent.name != '天五talent' &&
      player.talent.name != '垃圾五talent' &&
      player.talent.name != '九转轮回体' &&
      player.talent.name != '九重魔功'
    ) {
      e.reply(
        `你talent不齐，无成帝the资格！请先夺天地之造化，修补talent后再来突破吧`
      )
      return false
    }
    //凡人突破
  ...
  async yes(e) {
    ...
    if (choice == '先不突破') {
      ...
      this.setContext('yes')
      await this.reply(
        '突破后talent将被固化，无法使用【洗根水】进行洗髓！回复:【确认突破】或者【先不突破】进行选择'
      )
      return false
    }
    /** 结束上下文 */
  ...
  async fate_up(e) {
    ...
    if (player.talent.type == '伪talent') {
      y = 3
    } else if (player.talent.type == '真talent') {
      y = 6
    } else if (player.talent.type == '天talent') {
      y = 9
    } else if (player.talent.type == '体质') {
      y = 10
    } else if (player.talent.type == '转生' || player.talent.type == '魔头') {
      y = 21
    } else if (player.talent.type == '转圣') {
      y = 26
    } else {
      y = 12
    }
